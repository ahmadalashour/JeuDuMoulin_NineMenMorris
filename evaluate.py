...
def main():
    ...
    # Evaluation Results 
    evaluation_folder = Path("./evaluation_results/{}".format(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))
        ...
        for difficulty_2 in range(1, 6):
            ...
            while True:  # Main game loop
                ...
                if board.game_over: 
                    ...
                    with open(evaluation_folder / "evaluation_results_{}_{}.txt".format(TRAINING_PARAMETERS["DIFFICULTY"]["orange"], TRAINING_PARAMETERS["DIFFICULTY"]["white"], datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")), "w") as f:
                        f.write("Winner : {}\n".format(board.winner))
