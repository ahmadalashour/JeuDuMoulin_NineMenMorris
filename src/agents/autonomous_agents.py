...
from src.game_env.board import Board  # Import the Board class used for all operations taht take place on the game board.
from src.globals import NODE_LOOKUP, TRAINING_PARAMETERS, EVALUATION_COEFFICIENTS  # Configuration constants used throughout the agent's decision-making.
from typing import Optional, Any  # Provides type hinting to enhance code readability and debugging.
import numpy as np  # Used for mathematical operations and random sampling, used especially for the agent's decision logic.
import dataclasses as dc  # Makes the use of decorators easier to create data classes, used here for the MinMaxAgent.
from copy import deepcopy  # Ensures deep copies of objects for safe manipulation without altering the originals (because we had some issues with shallow copies).
from src.game_env.node import Node  # Import the Node class used for representing placing positions on the board.
from multiprocessing import Pool, cpu_count  # Enables parallel processing to speed up the Minimax computation for high difficulties.
import signal  # Allows handling of interrupt signals, useful for safely terminating multiprocessing tasks.
import time  # Used for handling time-based operations, like simulating move delays.



@dc.dataclass # Decorator to create a data class for the MinMaxAgent.
class MinMaxAgent:
    ...
    max_n_samples: Optional[int] = None  # Maximum samples for decision-making, can be None if not used.
    render_steps: int = 30  # Number of steps for rendering animations of moves.
    ...
    def generate_possible_moves(board: Board) -> list[tuple[int | None, "Node", int]]:
        ...
        """
        generated_moves = [] # Initialize a list to store possible moves.
        ...
        # Handle move generation during the 'moving' phase of the game.
            ...
            for piece in board.pieces[board.turn]:
                for node in (
                    ...
                    legality = piece.check_legal_move(board=board, new_node=node, just_check=True) # Check move legality for each potential node.
                    if legality in ["move", "remove"]:
        ...
        # Handle move generation during the 'capturing' phase of the game.
            ...
            # Determine the opponent's turn and generate capture moves for removable pieces.
    ...
    def make_move(
        ...
        # If a piece is found, attempt to execute the move.
    ...
    def minimax(
        ...
        if multicore == 1 or depth == 1 or len(possible_moves)/cpu_count() < 0.5:
            # Sequential processing of each move.
